...
export const useAboutPageContents = () => {
  const [contents, setContents] = useState<AboutPageContent[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  const fetchContents = async () => {
    try {
      const { data, error } = await supabase
        .from('about_page_contents')
        .select('*')
        .eq('is_active', true)
        .order('display_order');
      ...
      const typedData: AboutPageContent[] = (data || []).map(item => ({
        ...item,
  ...
  const updateContent = async (id: string, updates: Partial<AboutPageContent>) => {
    try {
  ...
  const uploadImage = async (file: File, blockKey: string) => {
    try {
      ...
      const fileName = `about/${blockKey}-${Date.now()}.${fileExt}`;
